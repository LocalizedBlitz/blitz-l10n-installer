<script lang="ts">
  ...
  let settingsRefreshKey: number = $state(0);
  let lang: Language = $state('zh_CN');
  let theme: 'light' | 'dark' = $state('light');

  let gBlitzPaths: string[] = $state([]);
  let gRunningStates: Record<string, boolean> = $state({});
  ...
  async function checkRunning() {
    for (const p of gBlitzPaths) {
      try {
        gRunningStates[p] = await invoke<boolean>('is_app_running', { installPath: p });
      } catch {
        gRunningStates[p] = false;
      }
    }
  }
  ...
  function onPathsChange(paths: string[]) {
    gBlitzPaths = paths;
  }
  ...
  async function loadConfig() {
    try {
      ...
      const config = await invoke<{ language: string; theme: string; blitz_paths: string[] }>('get_app_config');
      if (config.language) {
      ...
      if (config.blitz_paths) {
        gBlitzPaths = config.blitz_paths;
      }
...
<div class="flex flex-col h-screen" style="background: var(--bg);">
  ...
  <header class="flex items-center justify-between px-6 pt-5 pb-0 shrink-0">
    <nav class="tabs">
      ...
      <button class="tab" class:tab-active={activeTab === 1} onclick={() => { activeTab = 1; settingsRefreshKey++; }}>
        {t('menu.settings')}
      </button>
  ...
  <main class="flex-1 overflow-y-auto p-6">
    <div style="display: {activeTab === 0 ? 'contents' : 'none'}">
      <HomeTab {lang} runningStates={gRunningStates} blitzPaths={gBlitzPaths} {onPathsChange} {onRefreshRunning} />
    </div>
    <div style="display: {activeTab === 1 ? 'contents' : 'none'}">
      <SettingsTab {lang} {onLangChange} refreshKey={settingsRefreshKey} />
    </div>
