<script lang="ts">
  ...
  let { lang = 'zh_CN', onLangChange, refreshKey = 0 }: { lang?: Language; onLangChange?: (lang: Language) => void; refreshKey?: number } = $props();
  ...
  $effect(() => {
    refreshKey; // re-run when tab becomes active
    loadConfig();
  });
</script>
