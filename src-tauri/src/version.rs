...
fn wide_to_string(ptr: *const u8, len: u32) -> String {
    ...
    let slice = unsafe {
        std::slice::from_raw_parts(ptr as *const u16, (len as usize).saturating_sub(1))
    };
...
pub fn get_product_version(exe_path: &Path) -> Option<String> {
    ...
    let result = unsafe {
        GetFileVersionInfoW(wide_path.as_ptr(), handle, info_size, buffer.as_mut_ptr())
    };
...
fn find_main_exe(dir: &Path) -> Option<std::path::PathBuf> {
    let exe = dir.join("tanksblitz.exe");
    if exe.exists() {
        return Some(exe);
    }
    // Fallback: find any .exe in the directory.
