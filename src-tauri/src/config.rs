...
pub struct AppConfig {
    ...
    #[serde(default)]
    pub blitz_paths: Vec<String>,
    #[serde(default)]
    pub blitz_lang_id: String,
}
...
pub fn build_proxy_url(config: &ProxyConfig) -> Option<String> {
    ...
    if let Some(ref username) = config.custom_username {
        if !username.is_empty() {
            let password = load_proxy_password().unwrap_or(None).unwrap_or_default();
